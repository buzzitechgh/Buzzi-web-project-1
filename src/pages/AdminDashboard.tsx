...
const AdminDashboard: React.FC = () => {
  ...
  const [newQuoteData, setNewQuoteData] = useState<Partial<QuoteFormData>>({ 
      name: '', email: '', phone: '', serviceType: 'General', grandTotal: 0, items: [], timeline: 'Immediate' 
  });
  ...
  const handleEditQuoteItem = (item: QuoteItem) => {
      ...
      // Remove the item from the list so it can be re-added/updated
      const updatedItems = (newQuoteData.items || []).filter(i => i.id !== item.id);
      const total = updatedItems.reduce((acc, i) => acc + (i.price * i.quantity), 0);
      setNewQuoteData({ ...newQuoteData, items: updatedItems, grandTotal: total });
  };

  const handleRemoveQuoteItem = (itemId: string) => {
      const updatedItems = (newQuoteData.items || []).filter(i => i.id !== itemId);
      const total = updatedItems.reduce((acc, i) => acc + (i.price * i.quantity), 0);
      setNewQuoteData({ ...newQuoteData, items: updatedItems, grandTotal: total });
  };
  ...
  const handleGenerateAdminQuote = async () => {
      ...
      const fullQuote: QuoteFormData = {
          ...
          timeline: newQuoteData.timeline || 'Immediate',
      ...
      try {
          ...
          await generateInvoice(fullQuote); 
          setQuotes(prev => [fullQuote, ...prev]);
          setIsCreatingQuote(false);
          setNewQuoteData({ name: '', email: '', phone: '', serviceType: 'General', grandTotal: 0, items: [], timeline: 'Immediate' });
