...
interface LogoProps {
  ...
  customSrc?: string | null; // For user uploaded logo
}

const Logo: React.FC<LogoProps> = ({ className = "h-10", lightMode = false, customSrc }) => {
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      {/* Icon: Use custom image if available, else SVG */}
      {customSrc ? (
        <img src={customSrc} alt="Company Logo" className="h-full object-contain max-h-12" />
      ) : (
